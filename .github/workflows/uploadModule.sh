...
wget --load-cookies=${COOKIES} --post-data="id=${MODULE_ID}" "http://developer.z-wave.me/?uri=moduleverify" -O /dev/null
if [ -n "${ADMIN}" ]; then
	wget --load-cookies=${COOKIES} --post-data="id=${MODULE_ID}&verifed=1" "http://developer.z-wave.me/?uri=adminmoduleverify" -O /dev/null
fi
